...
# @app.post("/token/", response={200: dict})
# def obtain_token_pair(request, payload: TokenObtainPairView):
#     user = authenticate(username=payload.username, password=payload.password)
#     if user is not None:
#         refresh = RefreshToken.for_user(user)
#         return {"refresh": str(refresh), "access": str(refresh.access_token)}
#     return app.create_response(request, {"error": "Invalid credentials"}, status=401)


@app.get("/devices/", response={200: list[DeviceSchema]}, tags=['Facade'])
def list_devices(request):
    user = request.user
    devices = Device.objects.filter(user=user)
    return devices

@app.get("/gatewayiot/{gateway_id}/jwt/", response={200: dict}, tags=['Facade'])
...
@app.post("/devices/{device_id}/rpc/", response={200: dict}, tags=['Facade'])
def call_device_rpc(request, device_id: int, payload: DeviceRPCView):
    ...
    return app.create_response(request, response.json(), status=response.status_code)


@app.get("/gatewaysiot/{gateway_id}/discover-devices/", response={200: list[DeviceSchema]}, tags=['Facade'])
...
@app.get("/devices/{device_id}/rpc-methods/", response={200: list}, tags=['Facade'])
...
@app.post("/gatewaysiot/", response={201: GatewayIOTSchema}, tags=['Facade'])
...
@app.get("/gatewaysiot/", response={200: list[GatewayIOTSchema]}, tags=['Facade'])
